                            ...
                            client.query("DELETE FROM results WHERE repo = '"+repo+"';", function (error, result){
                                if (error){
                                    ...
                                    if (result.rows.length === 0){
                                        ...
                                        client.query("UPDATE results SET total = "+accessibility[key].total+", errors = "+accessibility[key].errors+", warnings = "+accessibility[key].warnings+", notices = "+accessibility[key].notices+" WHERE url = '"+key+"';", function (err, res, done){
                                            if (err){
                                                console.log("#2: "+err);
                                            }
                                            done();

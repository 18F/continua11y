...
function report(githubData, travisData) {
    ...
    pg.connect(conString, function (err, client, done){
        if (err) {
            ...
            client.query("CREATE TABLE IF NOT EXISTS repo_"+githubData.id+"(repo_id int, commit text, total int, error int, warning int, notice int);", function (err, result){
                done();
            });
            ...
            async.forEachOfSeries(travisData.data, function (data, url, callback){
                ...
                if (err) {
                    ...
                    client.query("INSERT INTO results VALUES('"+travisData.repository+"', '"+githubData.id+"', '"+overall.total+"', '"+overall.error+"', '"+overall.warning+"', '"+overall.notice+"');", function (err, result){
                        done();
                        if (err) {
                            console.log(err);
                        ...
                        if (!result) {
                            client.query("UPDATE results SET (repo_name, total, error, warning, notice) = ('"+travisData.repository+"', '"+overall.total+"', '"+overall.error+"', '"+overall.warning+"', '"+overall.notice+"') WHERE repo_id = '"+githubData.id+"';", function (err, result){
                                done();
                                if (err) {
                    ...
                    client.query("INSERT INTO repo_"+githubData.id+" VALUES('"+githubData.id+"', '"+travisData.commit+"', '"+overall.total+"', '"+overall.error+"', '"+overall.warning+"', '"+overall.notice+"');", function (err, result){
                        done();
                        if (err) {

...
function report(githubData, travisData) {
    ...
    pg.connect(conString, function (err, client, done){
        if (err) {
            ...
            client.query("CREATE TABLE IF NOT EXISTS repo_"+githubData.id+"(repo_id int, branch text, commit text UNIQUE NOT NULL, total int, error int, warning int, notice int, timestamp TIMESTAMP NOT NULL);", function (err, result){
                done();
            });

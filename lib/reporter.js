...
var Reporter = {
  ...
  calculate: function (githubData, travisData, done) {
    ...
    databaseSaves.push((next) => {
      Reporter.saveCommit(githubData, travisData, sums, next);
    });

    databaseSaves.push((next) => {
      if (travisData.pull_request === 'false') {
        Reporter.saveRepo(githubData, travisData, sums, next);
      }
    });

    databaseSaves.push((next) => {
      Reporter.tellGitHub(githubData, travisData, sums, next);
    });
  ...
  saveCommit: function (githubData, travisData, overall, done) {
    done = done || function () {};
  ...
  saveRepo: function (githubData, travisData, overall, done) {
    done = done || function () {};
  ...
  tellGitHub: function (githubData, travisData, overall, done) {
    done = done || function() {};
    var context = '';
    var message = '';
    var state = '';

...
var Reporter = {
    ...
    calculate: function (githubData, travisData) {
        ...
        async.forEachOfSeries(travisData.data, function (data, url, callback){
            overall.total += data.pa11y.count.total;
            overall.error += data.pa11y.count.error;
            overall.warning += data.pa11y.count.warning;
            overall.notice += data.pa11y.count.notice;
            url = parse(url, true);
    ...
    saveURL: function (githubData, travisData, data, url) {
        ...
        models.Url.create({
            ...
            commit: travisData.commit,
            repo: githubData.id,
            total: data.pa11y.count.total,
            error: data.pa11y.count.error,
            warning: data.pa11y.count.warning,
            notice: data.pa11y.count.notice
        });
    },
